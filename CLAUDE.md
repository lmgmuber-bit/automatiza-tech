...
- **ARGOS Mecánico (2026-08-18):** agente n8n que repara automáticamente errores de configuración de workflows detectados por ARGOS (máx. 3 intentos, siempre notifica a Luis por correo). Spec en `.superpowers/specs/2026-08-16-argos-mecanico-design.md`, workflows exportados en `N8N/PROD/Argos Mecanico.json` y `N8N/PROD/Argos Mecanico - Aplicar Fix.json`. Solo toca configuración de n8n — nunca PHP/deploys. Arquitectura endurecida tras un incidente real (un intento de reparación corrompió un workflow de producción durante las pruebas; restaurado y corregido con un sub-workflow dedicado que hace merge determinístico del nodo en vez de dejar que el LLM genere el PATCH completo). Verificado en vivo contra el backlog real de ARGOS sin corrupción adicional. Rama `feat/argos-mecanico`, PR pendiente de crear/mergear a `main` (bloqueado por el clasificador de seguridad al intentar automatizarlo — requiere que Luis abra el PR manualmente desde `https://github.com/lmgmuber-bit/automatiza-tech/pull/new/feat/argos-mecanico`).
