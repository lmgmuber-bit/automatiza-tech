...
- Final: `public/themes/carreras/rayo-mcqueen-estrella.mp4`, H.264,
  720×1280, 30 fps, `yuv420p`, 5,000 s, AAC estéreo 44,1 kHz. La pista
  generada por Wan se normalizó para que sea audible y el frontend reduce la
  música temática a volumen ambiente mientras se reproduce el clip.
...
- QA final: frontend 81/81; backend 125/125 y lint 49 archivos + 8
  entrypoints en PHP 8.0/8.2/8.3/8.4; smoke HTTP 28/28; build Vite limpio;
  paridad `public→dist` de 283 archivos y recorrido Chrome hasta
  `FULL · CONCIERTO 3D` sin errores de consola. La aserción histórica que
  esperaba `mundo3d` fue actualizada al contrato `concierto3d` + `stage`.
