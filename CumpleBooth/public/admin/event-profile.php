  ...
  <main>
      ...
      <section class="card">
        ...
        <div id="people-list" class="profile-people" data-sort-list="people">
          ...
          <article class="profile-person" data-person data-sort-item>
            ...
            <div class="profile-grid profile-grid--2">
              <div class="field"><label>Nombre público o apodo</label><input aria-label="Nombre público o apodo" data-person-name="display_name" type="text" name="people[<?= $personIndex ?>][display_name]" maxlength="120" required value="<?= h($person['display_name'] ?? '') ?>"></div>
              <div class="field"><label>Apodo adicional (opcional)</label><input aria-label="Apodo adicional (opcional)" data-person-name="nickname" type="text" name="people[<?= $personIndex ?>][nickname]" maxlength="120" value="<?= h($person['nickname'] ?? '') ?>"></div>
              <div class="field profile-grid-span"><label>Presentación breve</label><textarea aria-label="Presentación breve" data-person-name="intro_text" name="people[<?= $personIndex ?>][intro_text]" maxlength="600" rows="3" placeholder="Cuenta algo breve, alegre y apropiado para los invitados."><?= h($person['intro_text'] ?? '') ?></textarea></div>
            </div>
    ...
    <section class="card profile-photo-card">
      ...
      <form method="post" enctype="multipart/form-data" action="event-profile.php?party=<?= rawurlencode($publicSlug) ?>" class="profile-upload-form">
        ...
        <div class="field"><label>Imagen JPG, PNG o WEBP (máx. 12 MB)</label><input aria-label="Imagen JPG, PNG o WEBP (máx. 12 MB)" type="file" name="photo" accept="image/jpeg,image/png,image/webp" required></div>
        <div class="field"><label>Texto alternativo</label><input aria-label="Texto alternativo" type="text" name="alt_text" maxlength="180" placeholder="Ej. Retrato sonriente de <?= h($person['display_name'] ?? '') ?>"></div>
    ...
    <section class="card profile-video-card">
      ...
      <form method="post" action="event-profile.php?party=<?= rawurlencode($publicSlug) ?>" class="profile-stack">
        ...
        <div class="profile-grid profile-grid--3">
          <div class="field"><label>Duración</label><select aria-label="Duración" name="duration_seconds"><option value="4">4 segundos</option><option value="5" selected>5 segundos</option><option value="6">6 segundos</option></select></div>
          <div class="field"><label>Modelo confirmado en models_explore</label><input aria-label="Modelo confirmado en models_explore" type="text" name="model_key" maxlength="100" placeholder="Completar después de consultar Higgsfield"></div>
          <div class="field"><label>Estilo emocional</label><input aria-label="Estilo emocional" type="text" name="emotional_style" maxlength="40" value="<?= h($profile['intro_style'] ?? $themePreset['intro_style'] ?? 'mágico') ?>"></div>
          <div class="field"><label>Costo exacto (créditos)</label><input aria-label="Costo exacto (créditos)" type="number" name="quote_credits" min="0" step="1" placeholder="Sin estimar"></div>
          <div class="field"><label>Costo exacto (monto)</label><input aria-label="Costo exacto (monto)" type="number" name="quote_amount" min="0" step="0.01" placeholder="0,00"></div>
          <div class="field"><label>Moneda</label><input aria-label="Moneda" type="text" name="quote_currency" maxlength="12" value="USD"></div>
          <div class="field"><label>Vigencia de la cotización (opcional)</label><input aria-label="Vigencia de la cotización (opcional)" type="datetime-local" name="quote_expires_at"></div>
          <div class="field profile-grid-span"><label>Frase/beat emocional</label><input aria-label="Frase/beat emocional" type="text" name="video_phrase" maxlength="160" value="<?= h($profile['intro_phrase'] ?? '') ?>"></div>
          <div class="field profile-grid-span"><label>Prompt final a revisar</label><textarea aria-label="Prompt final a revisar" name="prompt" rows="12" maxlength="12000"><?= h($defaultPrompt) ?></textarea><small>Debe mantenerse sin nombres de franquicias ni personajes protegidos. El texto público se superpone después en HTML.</small></div>
        </div>
...
<template id="person-template">
  <article class="profile-person" data-person data-sort-item>
    ...
    <div class="profile-grid profile-grid--2"><div class="field"><label>Nombre público o apodo</label><input aria-label="Nombre público o apodo" data-person-name="display_name" type="text" maxlength="120" required></div><div class="field"><label>Apodo adicional (opcional)</label><input aria-label="Apodo adicional (opcional)" data-person-name="nickname" type="text" maxlength="120"></div><div class="field profile-grid-span"><label>Presentación breve</label><textarea aria-label="Presentación breve" data-person-name="intro_text" maxlength="600" rows="3"></textarea></div></div>
