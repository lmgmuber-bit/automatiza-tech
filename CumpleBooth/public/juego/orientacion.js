/*
 * SUGIERE girar el dispositivo y entra a pantalla completa. Compartido por los juegos 3D.
 *
 * Sugiere, no obliga: el aviso trae un botón "Jugar así" y respeta esa decisión hasta que se
 * cierre la pestaña. La versión anterior solo se iba si el aparato se giraba de verdad, y
 * dejaba encerrado a quien prefiriera vertical o tuviera el giro bloqueado desde los ajustes
 * del sistema —que es más común de lo que parece—. Tampoco se fija ya la orientación.
 *
...
(function () {
  ...
  /*
   * El aviso SUGIERE girar, no obliga.
   *
   * La primera versión era una capa a pantalla completa que solo desaparecía al girar de
   * verdad: si alguien prefería jugar vertical —o si el aparato tenía el giro bloqueado desde
   * los ajustes, que es más común de lo que parece— quedaba encerrado sin salida. Ahora hay
   * un botón para seguir igual, y esa decisión se respeta hasta que se cierre la pestaña.
   *
   * Se guarda en `sessionStorage` y no en `localStorage` a propósito: es una preferencia del
   * momento, no de la persona. La próxima fiesta vuelve a sugerirlo, que es lo correcto
   * porque casi siempre girar SÍ se ve mejor.
   */
  var LLAVE_SEGUIR = 'cc-gira-asi-esta-bien';
  var descartado = false;   // respaldo cuando el navegador no deja usar sessionStorage

  function prefiereSeguirAsi() {
    if (descartado) { return true; }
    try { return sessionStorage.getItem(LLAVE_SEGUIR) === '1'; } catch (e) { return false; }
  }

  function recordarSeguirAsi() {
    descartado = true;
    try { sessionStorage.setItem(LLAVE_SEGUIR, '1'); } catch (e) { /* sin memoria: vale la variable */ }
  }
  ...
  function pedirPantallaCompleta() {
    ...
    // Antes acá se llamaba a `screen.orientation.lock('landscape')`. Se quitó: dejaba el
    // aparato clavado en apaisado y quien quisiera volver a vertical ya no podía. El aviso
    // sugiere girar; una vez girado, la orientación sigue siendo del que juega.
  }
  ...
  function crearCapa() {
    ...
    capa.innerHTML =
      '<div class="cc-gira__caja">' +
        ...
        '<button type="button" class="cc-gira__seguir">' + t.seguir + '</button>' +
      '</div>';

    capa.querySelector('.cc-gira__seguir').addEventListener('click', function (ev) {
      ev.stopPropagation();
      recordarSeguirAsi();
      revisar();
    });
  ...
  function revisar() {
    var mostrar = avisaGirar() && enVertical() && !prefiereSeguirAsi();
    crearCapa().classList.toggle('cc-gira--on', mostrar);
    // Con el aviso puesto no debe quedar nada desplazándose por detrás. Al descartarlo se
    // devuelve el scroll: si el aviso no obliga, tampoco puede dejar la página trabada.
