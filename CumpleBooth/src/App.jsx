...
// Cuando un clip dentro del juego trae su propio audio, la pista temática queda
// apenas como ambiente y la voz/efectos del video permanecen en primer plano.
const MUSIC_VOL_FOREGROUND_VIDEO = 0.02
...
function BoothApp() {
  ...
  const [foregroundVideoPlaying, setForegroundVideoPlaying] = useState(false)
  ...
  useEffect(() => {
    ...
    if (foregroundVideoPlaying) {
      audio.volume = MUSIC_VOL_FOREGROUND_VIDEO
      return
    }
  ...
  const reset = () => {
    setForegroundVideoPlaying(false)
    setInvitado(null)
    setPersonaje(null)
    setPhoto(null)
    setResult(null)
    ...
    <div className="app">
      ...
      {screen === 'juego' && (
        <Juego
          invitado={invitado}
          personaje={personaje}
          onStarVideoAudioChange={setForegroundVideoPlaying}
...
function Spinner({ onDone }) {
  ...
  const winIdx = useRef(Math.floor(Math.random() * n))
...
/* Presentación WOW previa a El Show 3D. Si el MP4 trae audio, se reproduce en
   primer plano y BoothApp baja la música del kiosco durante esos segundos. Si
   falta o no carga, el watchdog avanza sin bloquear la fila de invitados. */
function VideoJuegoEstrella({ src, personaje, onDone, onAudioStateChange }) {
  const videoRef = useRef(null)
  const doneRef = useRef(false)
  const [needsTap, setNeedsTap] = useState(false)
  ...
  }, [finish, onAudioStateChange, src, startPlayback])
 ...
 */
function Juego({ invitado, personaje, onDone, onStarVideoAudioChange }) {
  ...
  if (config.kind === 'concierto3d') {
    ...
    const runnerAtlasSrc = (charName && CHAR_RUN_ATLAS[charName]) || null
    if (!videoEstrellaVisto && THEME_FLOW.starVideoAppliesTo?.(charName)) {
      return (
        <VideoJuegoEstrella
          src={THEME_FLOW.starVideo}
          personaje={personaje}
          onAudioStateChange={onStarVideoAudioChange}
    ...
    return (
      <StageConcert3D
        ...
        bannerSrc={CONFIG.images.bienvenida}
...
function JuegoFichas({ config, invitado, personaje, onDone }) {
  ...
  if (!config.image) {
    ...
    return (
      <section className="screen juego juego--muneco">
        <div className="juego-panel">
...
function JuegoCopos({ config, invitado, personaje, onDone }) {
  ...
  return (
    <section className={`screen juego${combo >= 5 ? ' is-fuego' : ''}`}>
      <div className="juego-hud">
