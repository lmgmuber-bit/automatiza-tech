...
export default function StageConcert3D({
  ...
  }, [bannerSrc, charSrc, config.image, config.stage, runnerAtlasSrc, showSeconds])
  ...
  return (
    ...
    >
      ...
      {phase === 'intro' && !unavailable && (
        <div className="show3d-card">
          <p className="show3d-kicker">El escenario es tuyo, {invitado || 'estrella'}</p>
          <h2>{style.title}</h2>
          <p>
            {personaje?.name || 'Tu personaje'} sale a cantar y el estadio entero te está
            esperando. Toca cada nota <strong>justo cuando llega al aro</strong> de su color.
          </p>
          <div className="show3d-legend">
            <span style={{ background: LANE_HEX[0] }} />
            <span style={{ background: LANE_HEX[1] }} />
            <span style={{ background: LANE_HEX[2] }} />
          </div>
          <button className="cta pulse" onClick={start} disabled={!ready}>
