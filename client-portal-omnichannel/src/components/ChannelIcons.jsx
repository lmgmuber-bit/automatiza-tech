/**
 * SVG icons for known channel types.
 * Used by ChannelBadge, ChannelTypesView, ChannelsView, etc.
 */
...
/**
 * Returns an SVG icon for known channel slugs, or null if unknown.
 * @param {string} slug - Channel type slug (e.g. 'whatsapp', 'instagram')
 * @param {number} size - Icon size in px (default 20)
 */
export function getChannelIcon(slug, size = 20) {
  const fn = icons[slug?.toLowerCase()];
  return fn ? fn(size) : null;
}

/**
 * Renders channel icon (SVG if known, emoji fallback).
 * @param {string} slug - Channel type slug
 * @param {string} emoji - Fallback emoji
 * @param {number} size - Icon size in px
 */
export function ChannelIcon({ slug, emoji, size = 20 }) {
  const svgIcon = getChannelIcon(slug, size);
