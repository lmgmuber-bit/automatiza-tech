<?php
/**
 * Insert KellsCapilar prompt config for the DEMO Whatsapp AT channel.
 * Run ONCE in production, then DELETE this file.
 */

define('ABSPATH', '');
// Try same directory first (WordPress root), then one level up
$wp_load = file_exists(__DIR__ . '/wp-load.php')
    ? __DIR__ . '/wp-load.php'
    : __DIR__ . '/../wp-load.php';

if (!file_exists($wp_load)) {
    die('wp-load.php not found. Place this file in the WordPress root or adjust the path.');
}
...
// ── 1. Find the DEMO Whatsapp AT channel ──────────────────────────────────────
$channel = $wpdb->get_row(
    $wpdb->prepare(
        "SELECT id, channel_name FROM {$prefix}channels WHERE channel_name LIKE %s LIMIT 1",
        '%DEMO%Whatsapp%AT%'
    ),
    ARRAY_A
);
...
if (!$channel) {
    // Fallback: try any channel with "DEMO" in the name
    $channel = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT id, channel_name FROM {$prefix}channels WHERE channel_name LIKE %s LIMIT 1",
            '%DEMO%'
...
if (!$channel) {
    echo '<pre style="color:red">ERROR: No se encontró el canal DEMO Whatsapp AT en la tabla ' . $prefix . 'channels.</pre>';
    echo '<pre>Canales disponibles:</pre>';
    $all = $wpdb->get_results("SELECT id, channel_name FROM {$prefix}channels ORDER BY id", ARRAY_A);
    echo '<pre>' . print_r($all, true) . '</pre>';
    exit;
}

echo '<pre>Canal encontrado: [' . $channel['id'] . '] ' . esc_html($channel['channel_name']) . '</pre>';

// ── 2. Check if config already exists for this channel ────────────────────────
$existing = $wpdb->get_var(
    $wpdb->prepare(
        "SELECT COUNT(*) FROM {$prefix}prompt_configs WHERE channel_id = %d",
        $channel['id']
    )
);

if ($existing > 0) {
    echo '<pre style="color:orange">AVISO: Ya existe ' . $existing . ' configuración(es) para este canal. No se insertará nada para evitar duplicados.</pre>';
    echo '<pre>Si deseas reemplazarla, elimínala primero desde el portal Prompts Bot.</pre>';
    exit;
}

// ── 3. Build prompt_data JSON ─────────────────────────────────────────────────
$prompt_data = [
    ...
    'info_servicios'            => 'Tenemos 24 servicios disponibles. Los precios van desde $5.000 hasta $85.000. Las duraciones van de 15 a 320 minutos. Para consultas específicas puedo ayudarte.',
    'catalogo_servicios_detallado' => '',
    ...
    'ejemplo_conversacion'      => "**Cliente**: \"Necesito alisar mi cabello\"\n\n**Kells**: \"¡Hola! El alisado capilar elimina el rizo dejando tu cabello liso y brillante por 3-4 meses 💇✨ Tenemos opciones según tu largo: corto $45.000, medio $55.000 o largo $65.000.\n\n¿Te gustaría agendar una cita? La sesión dura aproximadamente 2-3 horas y debes llegar con el cabello recién lavado 📅\"",
];

// ── 4. Insert ─────────────────────────────────────────────────────────────────
$insert = [
    'channel_id'   => (int) $channel['id'],
    'config_name'  => 'KellsCapilar - Configuración Principal',
...
$result = $wpdb->insert($prefix . 'prompt_configs', $insert);

if ($result === false) {
    echo '<pre style="color:red">ERROR al insertar: ' . esc_html($wpdb->last_error) . '</pre>';
} else {
    $new_id = $wpdb->insert_id;
    echo '<pre style="color:green">✅ Configuración insertada correctamente.</pre>';
    echo '<pre>ID: ' . $new_id . '</pre>';
    echo '<pre>Canal: [' . $channel['id'] . '] ' . esc_html($channel['channel_name']) . '</pre>';
    echo '<pre>Config: KellsCapilar - Configuración Principal</pre>';
    echo '<pre>Campos cargados: ' . count($prompt_data) . '</pre>';
