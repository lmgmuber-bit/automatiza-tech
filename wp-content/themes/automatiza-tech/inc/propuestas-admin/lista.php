...
class AT_Propuestas_Lista extends WP_List_Table {
    ...
    protected function extra_tablenav($which) {
        ...
        if ($this->has_items()) {
            echo '<div class="alignleft actions"><button type="submit" name="at_borrar_marcadas" value="1" class="button at-borrar-marcadas">🗑️ Borrar marcadas</button></div>';
        }
...
function at_pa_render_lista(string $message): void {
    ...
    $_SERVER['REQUEST_URI'] = remove_query_arg(['action', 'action2', 'proposal_ids', '_wpnonce', '_wp_http_referer', 'delete_id', 'filtrar', 'bulk_action', 'at_borrar_marcadas'], $_SERVER['REQUEST_URI']);
