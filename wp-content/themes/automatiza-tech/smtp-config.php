...
function automatiza_tech_smtp_config($phpmailer) {
    ...
    // Credenciales SMTP - usar variables de entorno o configuración
    // Opción 1: Desde wp-config.php (RECOMENDADO)
    if (defined('SMTP_USER') && defined('SMTP_PASS')) {
        $phpmailer->Username = SMTP_USER;
        $phpmailer->Password = SMTP_PASS;
    } 
    // Opción 2: Correo del sitio
    else {
        $phpmailer->Username = get_option('admin_email');
        $phpmailer->Password = ''; // CONFIGURAR EN PRODUCCIÓN
    }
    
    // Configurar remitente
    $phpmailer->From     = get_option('admin_email');
    $phpmailer->FromName = 'Automatiza Tech';
    $phpmailer->addReplyTo('info@automatizatech.cl', 'Automatiza Tech');
